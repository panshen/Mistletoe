...
public class Target_DeletedTarget_Adapter extends Target_Adapter {
	public Target_DeletedTarget_Adapter(Context context, ArrayList<Target_Bean> data, ReadyGoooFactory workFactory) {
	...
	protected void updateShow(int position, View convertView) {
		try {
			...
			// 隐藏掉无用的按键
			mHolder.textView03.setVisibility(ImageView.VISIBLE);
			mHolder.textView03.setText("撤回");
			mHolder.textView04.setVisibility(ImageView.GONE);
			mHolder.textView05.setVisibility(ImageView.VISIBLE);
			mHolder.textView05.setText("彻底删除");
			// 选中了第几条
		...
		try {
			mHolder.textView03.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					try {
						bean.updateStatusCloud(getContext(), UpdateStatusRequestNumber.RESTORE_TARGET, null);
					} catch (Exception e) {
						ExceptionHandle.ignoreException(e);
					}
				}
			...
			mHolder.textView05.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					try {
						bean.updateStatusCloud(getContext(), UpdateStatusRequestNumber.ERASE_TARGET, null);
					} catch (Exception e) {
						ExceptionHandle.ignoreException(e);
					}
				}
