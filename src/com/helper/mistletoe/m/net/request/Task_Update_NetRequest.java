...
public class Task_Update_NetRequest extends Template_NetRequest {
	public Task_Update_NetRequest(Context context) {
		super(context, NetRequest_Bean.REQUEST_TYPE_POST, MainConst.NET_TASK_UPDATE);
	}
	...
	private String fromateData(Task_Bean task) {
		...
		try {
			...
			jData.put("task_id", (int) task.getTask_id());
			if (task.status != null) {
				jData.put("status", (int) task.getStatus().toInt());
			}
